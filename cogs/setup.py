...
def make_setup_embed(config: dict) -> discord.Embed:
    ...
    embed.add_field(name="Roles", value=(
        ...
        f"Suspended: {val('suspended_role')}\nStaff: {val('staff_role')}"
    ), inline=False)
...
class RolesModal(discord.ui.Modal, title="Configure Roles"):
    verified_role     = discord.ui.TextInput(label="Verified Role ID", required=False)
    unverified_role   = discord.ui.TextInput(label="Unverified Role ID", required=False)
    candidate_role    = discord.ui.TextInput(label="Candidate Role ID", required=False)
    leaderboard_role  = discord.ui.TextInput(label="Leaderboard Role ID", required=False)
    academy_pass_role = discord.ui.TextInput(label="Academy Pass Role ID", required=False)

    async def on_submit(self, interaction: discord.Interaction):
        for key, field in [
            ("verified_role", self.verified_role), ("unverified_role", self.unverified_role),
            ("candidate_role", self.candidate_role), ("leaderboard_role", self.leaderboard_role),
...
class ExtraRolesModal(discord.ui.Modal, title="Configure Extra Roles"):
    loa_role       = discord.ui.TextInput(label="LOA Role ID", required=False, placeholder="Role given during LOA")
    suspended_role = discord.ui.TextInput(label="Suspended Role ID", required=False, placeholder="Role given during suspension")
    staff_role     = discord.ui.TextInput(label="Staff Role ID", required=False, placeholder="Role with staff access to tickets")

    async def on_submit(self, interaction: discord.Interaction):
        for key, field in [
            ("loa_role", self.loa_role), ("suspended_role", self.suspended_role), ("staff_role", self.staff_role),
        ]:
            if field.value.strip():
                await set_config(key, field.value.strip())
        await interaction.response.edit_message(embed=make_setup_embed(await get_all_config()))
...
class PhaseRolesModal(discord.ui.Modal, title="Configure Phase Roles"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        for key, field in [
            ("phase1_role", self.phase1), ("phase2_role", self.phase2), ("phase3_role", self.phase3),
            ("phase4_role", self.phase4), ("phase5_role", self.phase5),
        ]:
            if field.value.strip():
                await set_config(key, field.value.strip())
        await interaction.response.edit_message(embed=make_setup_embed(await get_all_config()))
...
class LogChannels1Modal(discord.ui.Modal, title="Log Channels (1/2)"):
    role_mod   = discord.ui.TextInput(label="Role Moderation Channel ID", required=False, placeholder="Manual role changes + who did it")
    chat_mod   = discord.ui.TextInput(label="Chat Moderation Channel ID", required=False, placeholder="All server messages")
    server_mod = discord.ui.TextInput(label="Server Moderation Channel ID", required=False, placeholder="Channel create/edit/delete/perms")
    mod_mod    = discord.ui.TextInput(label="Mod Moderation Channel ID", required=False, placeholder="Role create/edit/delete/perms")
    member     = discord.ui.TextInput(label="Member Logs Channel ID", required=False, placeholder="Member joins with approve/ban buttons")

    async def on_submit(self, interaction: discord.Interaction):
        for key, field in [
            ("log_role_mod_channel", self.role_mod), ("log_chat_mod_channel", self.chat_mod),
            ("log_server_mod_channel", self.server_mod), ("log_mod_mod_channel", self.mod_mod),
...
class LogChannels2Modal(discord.ui.Modal, title="Log Channels (2/2)"):
    update    = discord.ui.TextInput(label="Update Logs Channel ID", required=False, placeholder="Bot updates and startup")
    promotion = discord.ui.TextInput(label="Promotion Logs Channel ID", required=False, placeholder="/promote and /demote actions")

    async def on_submit(self, interaction: discord.Interaction):
        for key, field in [("log_update_channel", self.update), ("log_promotion_channel", self.promotion)]:
            if field.value.strip():
                await set_config(key, field.value.strip())
        await interaction.response.edit_message(embed=make_setup_embed(await get_all_config()))
...
class SetupView(discord.ui.View):
    ...
    @discord.ui.button(label="Extra Roles", style=discord.ButtonStyle.primary, emoji="🏷️", row=0)
    async def extra_roles_btn(self, interaction: discord.Interaction, _: discord.ui.Button):
        await interaction.response.send_modal(ExtraRolesModal())
