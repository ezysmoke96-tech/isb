...
def parse_duration(s: str) -> datetime.timedelta | None:
    ...
    if not match or not any(match.groups()):
        return None
    delta = datetime.timedelta(days=int(match.group(1) or 0), hours=int(match.group(2) or 0), minutes=int(match.group(3) or 0), seconds=int(match.group(4) or 0))
...
def giveaway_embed(prize: str, num_winners: int, end_at: int, ended: bool = False, winners: list[str] = None) -> discord.Embed:
    if ended:
        embed = discord.Embed(title="🎉 Giveaway Ended!", description=f"**Prize:** {prize}", color=discord.Color.greyple())
        embed.add_field(name="Winners", value=str(num_winners), inline=True)
        embed.add_field(name="🏆 Winner(s)", value="\n".join(f"<@{w}>" for w in winners) if winners else "No valid entries.", inline=False)
    else:
...
async def pick_and_announce_winners(bot: discord.Client, giveaway: dict, reroll: bool = False):
    ...
    valid = [uid for uid in entries if (m := guild.get_member(int(uid))) and not m.bot]
...
class GiveawayView(discord.ui.View):
    ...
    @discord.ui.button(label="🎉 Enter", style=discord.ButtonStyle.primary, custom_id="giveaway_enter_persistent")
    async def enter(self, interaction: discord.Interaction, button: discord.ui.Button):
        giveaway = await get_giveaway(str(interaction.message.id))
        if not giveaway:
            await interaction.response.send_message("❌ This giveaway no longer exists.", ephemeral=True)
            return
        if giveaway["ended"] or int(time.time()) > giveaway["end_at"]:
...
class GiveawayCog(commands.Cog, name="Giveaway"):
    ...
    @app_commands.describe(duration="How long: 1d, 2h, 30m, 1h30m", prize="What is being given away", winners="Number of winners (default: 1)")
    @app_commands.checks.has_permissions(manage_guild=True)
    async def giveaway(self, interaction: discord.Interaction, duration: str, prize: str, winners: app_commands.Range[int, 1, 20] = 1):
        delta = parse_duration(duration)
        if delta is None:
            await interaction.response.send_message(embed=discord.Embed(title="❌ Invalid Duration", description="Use formats like `30m`, `2h`, `1d`.", color=discord.Color.red()), ephemeral=True)
            return
    ...
    @app_commands.command(name="gend", description="End the active giveaway in this channel early")
    @app_commands.checks.has_permissions(manage_guild=True)
    async def gend(self, interaction: discord.Interaction):
        ...
        if not giveaway:
            await interaction.response.send_message(embed=discord.Embed(title="❌ No Active Giveaway", description="There is no active giveaway in this channel.", color=discord.Color.red()), ephemeral=True)
            return
    ...
    async def greroll(self, interaction: discord.Interaction):
        ...
        if not giveaway:
            await interaction.response.send_message(embed=discord.Embed(title="❌ No Giveaway Found", description="No giveaway found in this channel.", color=discord.Color.red()), ephemeral=True)
            return
