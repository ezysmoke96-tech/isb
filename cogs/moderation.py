...
class Moderation(commands.Cog):
    ...
    async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: str = "No reason provided"):
        if member == interaction.user:
            await interaction.response.send_message(embed=discord.Embed(title="Action Blocked", description="You cannot kick yourself.", color=discord.Color.orange()), ephemeral=True)
            return
        if member.top_role >= interaction.guild.me.top_role:
            await interaction.response.send_message(embed=discord.Embed(title="Action Blocked", description="I cannot kick someone with an equal or higher role than me.", color=discord.Color.orange()), ephemeral=True)
            return
        try:
            await member.send(embed=discord.Embed(title="You have been kicked", description=f"**Reason:** {reason}\n**Server:** {interaction.guild.name}\n**By:** {interaction.user}", color=discord.Color.orange()))
        except Exception:
            pass
    ...
    @app_commands.describe(user_id="User mention, name, or Discord ID", reason="Reason for the ban", duration="Duration: 1d, 2h, 30m — or 'permanent' (default)")
    @app_commands.checks.has_permissions(ban_members=True)
    async def ban(self, interaction: discord.Interaction, user_id: str, reason: str = "No reason provided", duration: str = "permanent"):
        ...
        if target is None:
            await interaction.followup.send(embed=discord.Embed(title="❌ User Not Found", description=f"Could not find user `{user_id}`.", color=discord.Color.red()), ephemeral=True)
            return
        ...
        dm_embed = discord.Embed(title="🚫 You have been banned!", color=0x8B0000)
        dm_embed.description = f"You have been banned for **{reason}** — Duration: **{dur_label}**, by **{interaction.user}**."
        ...
        try:
            ...
            pass
        await interaction.guild.ban(discord.Object(id=target.id), reason=f"{reason} — by {interaction.user}", delete_message_days=0)
        if delta:
    ...
    @app_commands.describe(user_id="Discord user ID or username to unban")
    @app_commands.checks.has_permissions(ban_members=True)
    async def unban(self, interaction: discord.Interaction, user_id: str):
        ...
        if target_user is None:
            await interaction.followup.send(embed=discord.Embed(title="❌ User Not Found", description=f"Could not find `{user_id}` in the ban list.", color=discord.Color.red()), ephemeral=True)
            return
        try:
            await interaction.guild.unban(target_user, reason=f"Unbanned by {interaction.user}")
        except discord.NotFound:
            await interaction.followup.send(embed=discord.Embed(title="❌ Not Banned", description=f"{target_user} is not currently banned.", color=discord.Color.orange()), ephemeral=True)
            return
    ...
    @app_commands.describe(member="The member to mute", duration="Duration: 1d, 2h, 30m (max 28 days)", reason="Reason")
    ...
    async def mute(self, interaction: discord.Interaction, member: discord.Member, duration: str, reason: str = "No reason provided"):
        ...
        if delta is None:
            await interaction.response.send_message(embed=discord.Embed(title="❌ Invalid Duration", description="Use a format like `30m`, `2h`, `1d`.", color=discord.Color.red()), ephemeral=True)
            return
        if delta > datetime.timedelta(days=28):
            await interaction.response.send_message(embed=discord.Embed(title="❌ Duration Too Long", description="Discord timeout cannot exceed **28 days**.", color=discord.Color.red()), ephemeral=True)
            return
        until = discord.utils.utcnow() + delta
        await member.timeout(until, reason=f"{reason} — by {interaction.user}")
        dur_label = duration_label(duration)
        try:
            await member.send(embed=discord.Embed(title="🔇 You have been muted", description=f"**Server:** {interaction.guild.name}\n**Duration:** {dur_label}\n**Reason:** {reason}\n**By:** {interaction.user}", color=discord.Color.orange()))
        except Exception:
            pass
